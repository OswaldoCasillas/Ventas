...
Formato aceptado del cuerpo del Issue (ambos soportados):
  Fecha: YYYY-MM-DD
  Notas: ...
  Items
  SKU | Cantidad | Precio
...
Si no encuentra fecha en el cuerpo, la toma del título:
...
def load_menu():
    """
    Carga docs/menu.json (o menu.json en raíz) para mapear:
      SKU -> {item, descripcion, precio}
    """
...
def parse_issue_payload(issue: dict):
    ...
    # 2) Fallback: fecha en el título (“… @ YYYY-MM-DD”)
    if not fecha:
        mt = DATE_RX_TITLE.search(title)
        if mt:
            fecha = mt.group(1)

    # 3) Localizar bloque de Items
    ...
    if start_idx is None:
        start_idx = 0  # fallback: buscar cualquier línea con pipes
    ...
    for ln in lines[start_idx:]:
        ...
        # Separadores como "----|----" o similares
        if set("".join(row)) <= {"-", " ", ":"}:
            continue

        # Líneas tipo "**Cantidad**:" que a veces aparecen
...
def write_daily_csv(base_dir: Path, fecha: str, rows: list):
    ...
    Escribe/append en docs/diario/YYYY-MM-DD.csv (o mercado/diario).
    """
    ensure_dir(base_dir)
...
def process_issue_event(event_path: Path):
    ...
    for it in items:
        ...
        # Precio unitario: el de la línea, o el del menú si viene vacío/0
